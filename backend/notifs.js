...
async function check(user, users_DS, notif_ds, settings_ds) {
  ...
  try {
    const [button_accept] = await page2.$x("//button[contains(., 'Turn On')]"); //click on save info
    if (button_accept) {
      await button_accept.click();
      page2.waitFor(2500)
    }
    else {
      console.log("pas le bouton");
    }
  ...
  try {
    ...
    const elements_x = await page2.$$("span.R19PB")
    for (i in elements_x) {
      console.log(i);
  ...
  try {
    ...
    const elements_y = await page2.$$("span.R19PB")
  ...
  try {
    //Partie mentions
    await page2.goto("https://www.instagram.com/accounts/activity/")
    await page2.waitFor(5000)
